...
class NeuralAgent():
        ...
        for i_episode in range(episode_count):
            ...
            for j_iter in range(max_steps):
                if tree:
                    ...
                    action_action = utils.clip_to_range(act_tree[0][0], ACTION_MIN[0], ACTION_MAX[0])
                else:
                    action_action = utils.clip_to_range(action_prog.pid_execute(window_list), ACTION_MIN[0], ACTION_MAX[0])
    ...
    def collect_data(self, controllers, tree=False):
        ...
        logging.info(f"{ENV_NAME} collection start with Lambda {self.lambda_mix}")
        ob, _ = env.reset()
    ...
    def label_data(self, controllers, observation_list, tree=False):
        ...
        for window_list in observation_list:
            if tree:
                ...
                net_obs_list.append(net_obs[:STATE_DIMS])
